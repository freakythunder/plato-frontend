...
const Language: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (storedAllTopics) {
      ...
      if (storedLanguage) {
        console.log(
          `[Initial Load] Checking topic synchronization for ${storedLanguage}`
        );

        const currentTopics = JSON.parse(localStorage.getItem("topics")) || [];
        console.log(
          "[Initial Load] Current topics from storage:",
          currentTopics
        );

        const languageIndex = parsedTopics.findIndex(
          (t) => t.language === storedLanguage
        );
        console.log(
          "[Initial Load] Language index in allTopics:",
          languageIndex
        );
        ...
        if (languageIndex !== -1) {
          ...
          console.log(
            "[Initial Load] Existing topics in allTopics:",
            existingTopics
          );

          // Compare topics using stringify for deep equality check
          if (
            JSON.stringify(existingTopics) !== JSON.stringify(currentTopics)
          ) {
            console.log(
              "[Initial Load] Topics mismatch detected, updating allTopics"
            );
            ...
            console.log(
              "[Initial Load] Topics are synchronized, no update needed"
            );
          }
        } else {
          if (storedLanguage !== "DSA_Practice") {
            console.warn(
              "[Initial Load] Stored language not found in allTopics"
            );
      ...
      const others = ["Python", "Java", "JavaScript", "C++", "DSA"].filter(
        (lang) => !courses.includes(lang)
      );
  ...
  const handleLanguageClick = async (language) => {
    ...
    if (storedLanguage === language) {
      navigate("/newpage", { state: { refresh: true } });
      return;
    }
    ...
    try {
      if (myCourses.includes(language)) {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/language/select`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDSAPracticeClick = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/dsa/allproblemsets`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className={styles.innerWrapper}>
            ...
            {myCourses.length === 0 ? (
              <div className={styles.welcomeMessage}>
                ...
                <div className={styles.gridContainer}>
                  {["Python", "Java", "JavaScript", "C++", "DSA"].map(
                    (language) => (
                      <div
                        key={language}
                  ...
                  {myCourses.map((course) => (
                    ...
                    >
                      ...
                      <span className={styles.completionText}>
                        {completions[course]
                          ? `${completions[course].completed}/${completions[course].total} subtopics completed`
                          : "0% Completed"}
                      </span>
                    </div>
                  ))}
        ...
        <div className={styles.dsaContainer} onClick={handleDSAPracticeClick}>
          <div className={styles.dsaCard}>
            ...
            <p className={styles.dsaCardDescription}>
